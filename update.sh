...
nginxv=$(nginx -v)
	...
	if [ "$nginxv" == "${NGINX_VERSION}" ]
		echo "${error} The newest Nginx Version is already installed" | awk '{ print strftime("[%H:%M:%S] |"), $0 }'
		exit 1
	fi
fi
...
if [ "$NGINX_UPDATE" = '1' ]; then
	...
	if [[ $endoutput != $NGINX_VERSION ]]; then
        ...
        echo "The newest Nginx Version is already installed... skipping Nginx Update"
	fi
fi 
